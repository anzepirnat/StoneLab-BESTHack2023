...
def dodaj(df, poraba, avto, tabela1, tabela2):
    #danes = today().strftime('%d-%m-%Y')
    n = df[df['nov']==avto.split()[0]].index[0]
    vrstica = df.iloc[n]
    vse = vrstica['CO2 Emissions(g/km)']*poraba/vrstica['Fuel Consumption Comb (L/100 km)']

    tabela1.append(datetime.datetime.today().strftime('%d-%m-%Y'))
    tabela2.append(vse)
...
final_transport = pickle.load(open('./data/food/final_food.pkl', 'rb'))
...
if mode == 'Carbon footprint overview 🦶🏿':
    ...
    tab1.subheader('Improve carbon footprint by consuming less:')
    ...
    path = './data/transport/CO2_Emissions_Canada.csv'

    df = pd.read_csv(path)
    df['nov'] = df['Make'].str.split().str[0]

    avti = list(df['Make'] + ' ' + df['Model'])

    pravi= [*set(avti)]
    pravi.sort()

    option = tab3.selectbox(
        'Izberite vaš avto:',
        (tuple(pravi))
    )
    ...
    #datumi = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
    vrstica = []
    stolpec = []
    datumi = []
    ...
    for single_date in daterange(start_date, end_date):
        datumi.append(single_date.strftime("%d-%m-%Y"))

    for datum in datumi:
        stolpec.append(random.randint(0,54))

    dodaj(df, 3, option, datumi, stolpec)



    source = pd.DataFrame({
            'CO2 emissions': stolpec,
            'Date':datumi
        })
    ...
    tab3.altair_chart(bar_chart, use_container_width=True)